    ...
    <div className="p-8">
      <div className="mb-8">
        <div className="flex items-center gap-2 mb-1">
          ...
          <span className="text-xs font-semibold text-brand-400 uppercase tracking-wider">Analytics</span>
        </div>
        ...
        <>
          ...
          <div className="grid grid-cols-2 gap-6 mb-6">
            ...
            <div className="card p-6">
              <h3 className="text-sm font-semibold text-white mb-5">Failure Code Distribution</h3>
              <ResponsiveContainer width="100%" height={240}>
                <BarChart data={failureCodeData} layout="vertical" margin={{ left: 10, right: 20 }}>
                  <CartesianGrid strokeDasharray="3 3" stroke="#1e293b" horizontal={false} />
                  <XAxis type="number" tick={{ fill: '#64748b', fontSize: 11 }} axisLine={false} tickLine={false} />
                  <YAxis dataKey="code" type="category" tick={{ fill: '#94a3b8', fontSize: 10 }} axisLine={false} tickLine={false} width={120} />
                  <Tooltip content={<CustomTooltip />} />
                  <Bar dataKey="count" fill="#6272f3" radius={[0, 4, 4, 0]} name="Count" />
                </BarChart>
              </ResponsiveContainer>
            </div>
            ...
            <div className="card p-6">
              <h3 className="text-sm font-semibold text-white mb-5">Recovery Status Distribution</h3>
              <ResponsiveContainer width="100%" height={240}>
                <PieChart>
          ...
          <div className="grid grid-cols-3 gap-6 mb-6">
            {/* Risk Level by Count */}
            <div className="card p-6">
              <h3 className="text-sm font-semibold text-white mb-5">Payments by Risk Level</h3>
            ...
            <div className="card p-6">
              <h3 className="text-sm font-semibold text-white mb-5">Revenue at Risk by Level</h3>
              <ResponsiveContainer width="100%" height={200}>
                <BarChart data={riskDistribution}>
                  <CartesianGrid strokeDasharray="3 3" stroke="#1e293b" />
                  <XAxis dataKey="level" tick={{ fill: '#64748b', fontSize: 11 }} axisLine={false} tickLine={false} />
            ...
            {/* Action Distribution */}
            <div className="card p-6">
              <h3 className="text-sm font-semibold text-white mb-5">Recovery Actions Taken</h3>
              {actionData.length === 0 ? (
          ...
          {(metrics?.revenue_by_day?.length || 0) > 0 && (
            <div className="card p-6">
              <h3 className="text-sm font-semibold text-white mb-5">Revenue Recovery Trend (Last 7 Days)</h3>
              <ResponsiveContainer width="100%" height={200}>
          ...
          <div className="card mt-6 overflow-hidden">
            <div className="px-6 py-4 border-b border-slate-800">
              <h3 className="text-sm font-semibold text-white">Recovery Summary by Status</h3>
            </div>
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-slate-800">
                    ...
                    <th className="px-6 py-3 text-right text-xs font-medium text-slate-500 uppercase tracking-wider">Count</th>
                ...
                <tbody className="divide-y divide-slate-800/60">
                  {[
                    { label: 'Recovered', count: metrics?.successful_recoveries || 0, amount: metrics?.recovered_revenue || 0, color: 'text-emerald-400' },
                    { label: 'Pending', count: metrics?.total_failed_payments || 0, amount: payments.filter(p => p.recovery_status === 'pending').reduce((s, p) => s + p.amount, 0), color: 'text-slate-400' },
                    { label: 'Escalated', count: metrics?.escalated_payments || 0, amount: payments.filter(p => p.recovery_status === 'escalated').reduce((s, p) => s + p.amount, 0), color: 'text-amber-400' },
                    ...
                    return (
                      <tr key={row.label} className="hover:bg-slate-800/20">
                        ...
                        <td className="px-6 py-3 text-right text-slate-300">{row.count}</td>
                        <td className="px-6 py-3 text-right text-slate-300">{fmt(row.amount)}</td>
                        <td className="px-6 py-3 text-right text-slate-400">{pct}%</td>
                      </tr>
                    )
                  })}
                </tbody>
